...
class DatabaseActionSequenceEditor {
    ...
    show(container,sequence){
        this.dispose();this.stepPlayback=null;this.sequence=sequence;this.selected=0;this.frame=0;this.undo=[];this.redo=[];this.cast={user:1,target:1};this.castKinds={user:'actors',target:'enemies'};this.mirrored=false;this.images={};
        ...
        const template=U.select(B.templates.map(t=>[t,t]),B.templates[0],()=>{});toolbar.append(template,U.button('Use Template',()=>{this.edit(()=>sequence.steps=B.template(template.value).steps);this.selected=0;this.drawSteps();this.drawInspector();}));
        ...
        const modes=U.element('div','rr-battle-toolbar');modes.append(U.button('Steps',()=>{this.timelineMode=false;this.drawSteps();}),U.button('Timeline',()=>{this.timelineMode=true;this.drawSteps();}));timeline.body.append(modes);
        this.steps=U.element('div','rr-sequence-steps');timeline.body.append(this.steps);this.stepLanguageChanged=()=>{this.updateStepDescriptions();this.drawInspector();this.validate();this.updateReferences();};window.addEventListener('rr-language-changed',this.stepLanguageChanged);
        const add=U.element('div','rr-battle-toolbar');this.stepType=U.select([...B.basicSteps.map(t=>['basic:'+t,t]),...B.types.map(t=>[t,this.label(t)])],'basic:Run to Target',()=>{});const addButton=U.button('Add Step',()=>this.insertSteps(this.newSteps()));add.append(this.stepType,addButton);timeline.body.append(add);this.bindStepEditing(timeline.panel,addButton);
    ...
    updateReferences(){const refs=ReactorBattleData.references(this.ui.settings(),this.sequence.id);this.ui.setText(this.references,refs.length?this.ui.message('Used by: {records}',{records:refs.map(r=>(window.I18n?.tDbType(r.kind)||r.kind)+' #'+r.id).join(', ')}):'Assign this sequence from a skill, item, weapon, actor or enemy. Previewing never applies damage or changes game state.');}
    motionLabels(){return {idle:'Idle',run:'Run',walk:'Walk',punch:'Punch',attack:'Attack',cast:'Cast',guard:'Guard',damage:'Damage',evade:'Evade',victory:'Victory',escape:'Escape'};}
    label(type){return {move:'Move / Position Key',motion:'Battler Motion',sound:'Play Sound',animation:'Show Animation',projectile:'Projectile',weapon:'Weapon Icon',impact:'Apply Action Effect',wait:'Wait',camera:'Camera Key'}[type]||type;}
    ...
    validate(){const errors=ReactorBattleData.validateSequence(this.sequence);this.validation&&(this.validation.textContent=errors.map(error=>this.ui.text(error)).join(' ')||this.ui.text('Ready to use. Apply Action Effect uses the skill’s existing targeting, damage and repeats.'));this.updateStepDescriptions();}
    ...
    previewSequence(){
        ...
        return index===undefined?this.sequence:{...this.sequence,steps:this.sequence.steps.slice(0,index+1)};
    }
    playStep(){
    ...
    newSteps(value=this.stepType.value){const B=ReactorBattleData;return value.startsWith('basic:')?B.basic(value.slice(6)):[B.step(value,value==='weapon'?{duration:0,x:.5,z:1}:{})];}
    ...
    drawInspector(){if(this.controls?.mode==='formation'){this.controls.inspectFormation();return;}const U=this.ui,step=this.sequence.steps[this.selected];this.inspector.replaceChildren();this.setInspectorTitle(step?this.label(step.type):'Action Steps');if(!step)return;
        const change=(key,value)=>this.edit(()=>step[key]=value);
        ...
        U.field(this.inspector,'Battler',U.select([['user','User'],['target','Current Target'],['allTargets','All Targets'],...Array.from({length:Math.max(this.targetCount,(step.targetIndex??0)+1)},(_,i)=>['target'+i,U.message('Target {n}',{n:i+1})])],ReactorBattleData.roleKey(step),v=>{this.edit(()=>{step.role=v.startsWith('target')&&v!=='target'?'target':v;step.targetIndex=v.startsWith('target')&&v!=='target'?Number(v.slice(6)):undefined;});this.drawInspector();}));
        if(['move','camera'].includes(step.type)){
            ...
            if(step.type==='move')U.field(this.inspector,'Facing',U.select([['','Keep Facing'],['movement','Direction of Travel'],['target','Target'],['home','Home Facing']],step.face||'',v=>change('face',v||undefined)));
            if(step.type==='move'){for(const key of ['rotateX','rotateY','rotateZ'])U.number(this.inspector,U.message('Rotation {axis} (degrees)',{axis:key.slice(-1)}),step[key]||0,v=>change(key,v),1);U.number(this.inspector,'Scale',step.scale??1,v=>change('scale',Math.max(.01,Math.min(100,v))));}
            U.field(this.inspector,'Easing',U.select([['smooth','Smooth'],['linear','Linear']],step.easing,v=>change('easing',v)));
        }
        if(step.type==='motion'){U.field(this.inspector,'Motion',U.select(Object.entries(this.motionLabels()),step.motion||'idle',v=>change('motion',v)));this.controls.transformFields(step,change);}
        if(step.type==='animation'){
            ...
            for(const key of ['x','y','z','scale'])U.number(this.inspector,key==='scale'?'Scale':U.message('{axis} (tiles)',{axis:key.toUpperCase()}),step.animationTransform?.[key]??(key==='scale'?1:0),v=>change('animationTransform',{...step.animationTransform,[key]:Math.max(key==='scale'?.01:-1000,Math.min(key==='scale'?100:1000,v))}));
        }
        if(step.type==='sound'){
        ...
        if(step.type==='weapon'){
            ...
            U.field(this.inspector,'Icon',U.select([['weapon','Equipped Weapon'],['action','Skill / Item'],['icon','Choose Icon']],step.iconSource||'weapon',v=>{change('iconSource',v);this.drawInspector();}));
            if(step.iconSource==='icon')this.inspector.append(U.button('Choose Icon…',()=>this.parent.showIconPicker(step.iconIndex||0,id=>change('iconIndex',id),require('path').join(this.parent.currentProject.path,'img','system','IconSet.png'))));
            for(const key of ['x','y','z'])U.number(this.inspector,U.message('{axis} (tiles)',{axis:key.toUpperCase()}),step[key]??0,v=>change(key,v));
            U.number(this.inspector,'Rotation (degrees)',step.rotation||0,v=>change('rotation',v),1);U.number(this.inspector,'Scale',step.scale||1,v=>change('scale',Math.max(.01,v)));
        }
        if(step.type==='projectile'){const color=U.element('input');color.type='color';color.value=step.color||'#ffcc55';color.onchange=()=>change('color',color.value);U.field(this.inspector,'Color',color);U.number(this.inspector,'Size (pixels)',step.size||8,v=>change('size',Math.max(1,v)),1);}
    ...
    async loadCast(){
        ...
        this.controls?.disposeGizmos();this.grid?.geometry.dispose();this.grid?.material.dispose();this.grid=null;this.preview?.dispose();this.preview=null;this.images={};this.models={};
        try {
            ...
            settings.projection='3d';settings.cameraSource='custom';settings.camera={x:7.5,y:5,z:1,yaw:0,pitch:25,distance:19};
            ...
            for(const role of ['user','target']){
                ...
                const spec=RRDatabase3DBindings.get(project.path,actor?'actors':'enemies',id,actor?'battler':undefined);
                ...
                else if(record.battlerName){const bitmap=await assets.image(actor?'sv_actors':'enemies',record.battlerName);this.images[role]={bitmap,actor};}
    ...
    advancePreview(delta){
        const B=ReactorBattleData,end=this.stepPlayback?.end??B.duration(this.sequence);
    ...
    previewCues(before,after){
        ...
        for(const [index,cue] of ReactorBattleData.timeline(this.previewSequence()).entries())if((!this.stepPlayback||index===this.stepPlayback.index)&&!this.previewFired.has(cue.step.id)&&cue.start>=before&&cue.start<=after){
            ...
            if(cue.step.type==='animation'&&cue.step.animationId){
                ...
                for(const key of keys){const ticket=this.preview?.playAnimation(key,cue.step.animationId,cue.step.animationTransform);if(ticket)tickets.push(ticket);else {
                    const animation=this.db.getAnimation(cue.step.animationId);
    ...
    motionFor(role){
        let result={name:'idle',start:0};for(const cue of ReactorBattleData.timeline(this.previewSequence())){if(cue.start>this.frame)break;if(cue.step.type==='motion'&&(ReactorBattleData.roleKey(cue.step)===role||role==='target0'&&ReactorBattleData.roleKey(cue.step)==='target'||role.startsWith('target')&&cue.step.role==='allTargets'))result={name:cue.step.motion||'idle',start:cue.start};}return result;
    }
    paint(){this.controls.resize();const B=ReactorBattleData,ctx=this.canvas.getContext('2d'),context=this.previewContext(),width=this.canvas.width,height=this.canvas.height,poses=Object.fromEntries(Object.entries((this.controls.mode==='formation'?context.homes:B.evaluate(this.previewSequence(),this.frame,context))).map(([k,p])=>[k,B.visualPose(p)]));
        ...
        if(view){
            ...
            for(const key of ['user','target0','target1','target2','target3']){
                ...
                if(image&&p){const {bitmap,actor}=image,w=bitmap.width/(actor?9:1),h=bitmap.height/(actor?6:1);
                    const index=({idle:1,run:0,walk:0,punch:6,attack:6,cast:9,guard:3,damage:4,evade:5,victory:16,escape:0})[motion.name]??1,pattern=Math.floor(Math.max(0,this.frame-motion.start)/12)%3;
                    view.billboard(key,bitmap.image,{x:actor?(Math.floor(index/6)*3+pattern)*w:0,y:actor?index%6*h:0,width:w,height:h},{...p,flipX:actor?p.facing>0:p.facing<0},2.5);
                }
                const record=view.models.get(key)||view.billboards.get(key);if(record?.object){record.object.visible=!!p;if(p){view.place(key,{...p,facing:p.facing??B.facingToward(p,role==='user'?poses.target:poses.user)});record.action=motion.name==='idle'?null:{name:motion.name,start:motion.start};if(motion.name==='idle'&&record.binding)record.binding.movingAt=undefined;}}
            }
            let weapon;for(const cue of B.timeline(this.previewSequence())){if(cue.start>this.frame)break;if(cue.step.type==='weapon')weapon=cue.step;}
            if(weapon&&weapon.visible!==false&&this.iconSet){const actor=this.castKinds.user==='actors'?this.db.getActor(this.cast.user):null,item=weapon.iconSource==='action'?this.db.getSkill(1):this.db.getWeapon(actor?.equips?.[0]),index=weapon.iconSource==='icon'?weapon.iconIndex||0:item?.iconIndex||0,p=poses.user;view.billboard('extra:weapon',this.iconSet.image,{x:index%16*32,y:Math.floor(index/16)*32,width:32,height:32},{x:p.x+(weapon.x??.5),y:p.y+(weapon.y||0),z:p.z+(weapon.z??1),rotateZ:weapon.rotation||0,scale:weapon.scale||1},.7);}else view.remove('extra:weapon');
            this.controls.sync(poses);view.seekAnimations=true;view.effectsPaused=!this.playing;view.frame=this.frame-1;view.render();ctx.drawImage(view.renderer.domElement,0,0,width,height);
            ...
            const active=B.timeline(this.previewSequence()).find(c=>c.start<=this.frame&&c.end>=this.frame&&c.step.type==='projectile');
            if(active){const t=(this.frame-active.start)/Math.max(1,active.step.duration),a=poses.user,b=poses.target,p=view.project({x:a.x+(b.x-a.x)*t,y:a.y+(b.y-a.y)*t,z:1});ctx.fillStyle=active.step.color||'#ffcc55';ctx.beginPath();ctx.arc(p.x,p.y,active.step.size||8,0,Math.PI*2);ctx.fill();}
