/**
 * Editor-only previews for native Reactor media-surface commands.
 * Preview state never enters map data or the runtime surface registry.
 */
class MediaSurfacePreviewManager {
    ...
    static _editorClass() {
        if (typeof MediaSurfaceEditor !== 'undefined') return MediaSurfaceEditor;
        if (typeof require === 'function') {
            try { return require('./event/commands/MediaSurfaceEditor.js'); } catch (_) {}
        }
        return null;
    }
    ...
    static scanMap(map) {
        ...
        for (const args of Array.isArray(map.reactor3d?.mediaSurfaces) ? map.reactor3d.mediaSurfaces : []) {
            if (!args || typeof args !== 'object') continue;
            const state = Editor.parse({code:357, parameters:['RPGReactor','ShowVideoSurface','',args]}, 'ShowVideoSurface');
            state.target = 'map'; state.wait = false;
            records.push({key:`map:${mapId}:${state.id}`, id:state.id, source:{mapId,surfaceId:state.id},
                ownerEventId:0, state, steps:[], stoppedAt:null, replacedAt:null});
        }
    ...
    beforeMapChange() {
        ...
        this.authoring = null;
        this.projectController?.mediaSurfaceManager?.close();
    ...
    rescan() {
        ...
        this.records = MediaSurfacePreviewManager.scanMap(this.map);
        this.refreshBackend();
    }
    ...
    refreshBackend() {
        ...
        this.syncToolInteraction();
    }

    /** Only the selected tool or an open authoring session owns surface input. */
    syncToolInteraction() {
        const enabled = this.ownsMapInput();
        for (const owner of this.pixiOwners.values()) if (owner.mesh) owner.mesh.eventMode = (owner.record?.authoring || enabled && !this.authoring) ? 'static' : 'none';
        for (const owner of this.domOwners.values()) if (owner.surface) owner.surface.style.pointerEvents = (owner.record?.authoring || enabled && !this.authoring) ? 'auto' : 'none';
    }
    ...
    _movieUrl(movie) {
        ...
        const Editor = MediaSurfacePreviewManager._editorClass();
        if (!root || !Editor?.safeMoviePath(movie) || typeof require !== 'function') return '';
        const path = require('path');
        const absolute = MediaSurfacePreviewManager._isImage(movie)
    ...
    _createMedia(state) {
        ...
        if (MediaSurfacePreviewManager._isImage(state.movie)) {
    ...
    attachPixi() {
        ...
        this.pixiBands = {};
        for (const band of MediaSurfacePreviewManager.PIXI_BANDS) {
            const root = new PIXI.Container();
            root.label = `Media Surface Previews (${band})`;
            root.sortableChildren = true;
            this.pixiBands[band] = root;
        }
        this.pixiMapRoot = this.pixiBands.mid;
        this.pixiScreenRoot = new PIXI.Container();
        this.pixiScreenRoot.label = 'Screen Media Surface Previews';
        ...
        // HD-2D render passes can leave PIXI's event boundary on an offscreen
        // root. Pick authoring handles against the visible map coordinates,
        // just as the 3D handles do, without redirecting other map input.
        this._pixiHandleInput?.removeEventListener('pointerdown', this._onPixiHandleDown, true);
        this._pixiHandleInput = app?.canvas;
        this._onPixiHandleDown = event => {
            if (event.button !== 0 || event.ctrlKey || event.altKey || !this.authoring) return;
            const owner = this.pixiOwners.get('__authoring__');
            if (!owner?.container?.visible || !owner.handles?.length) return;
            const point = this._clientToOwner(owner, event.clientX, event.clientY);
            if (!point) return;
            const anchor = this._anchor2d(owner.state, owner.record.ownerEventId);
            let nearest = null, distance = Infinity;
            owner.handles.forEach((handle,index) => {
                const d = Math.hypot(point.x-anchor.x-handle.x, point.y-anchor.y-handle.y);
                if (d <= 13 && d < distance) {nearest=owner.handleSpecs[index];distance=d;}
            });
            if (!nearest) return;
            event.stopImmediatePropagation();
            this._startPixiDrag(event, owner, nearest);
        };
        this._pixiHandleInput?.addEventListener('pointerdown', this._onPixiHandleDown, true);
    }
    ...
    _pixiRootFor(state) {
        ...
        return this.pixiBands?.[MediaSurfacePreviewManager.pixiBandFor(state.layer)] || this.pixiMapRoot;
    }
    ...
    _createPixiOwner(record) {
        ...
        try {
            ...
            console.warn('Could not create an editor media-surface preview:', error);
    ...
    _anchor2d(state, ownerEventId) {
        ...
        const lift = MediaSurfacePreviewManager.standingLift(state);
    ...
    _syncPixiScanlines(owner, corners) {
        const state = owner.state;
        const amount = MediaSurfacePreviewManager.scanlineAmount(state);
    ...
    _syncPixiHandles(owner, corners) {
        if (!owner.handles) {
            const specs = [
                { corners: [0], label: 'Top-left media surface corner' },
                { corners: [1], label: 'Top-right media surface corner' },
                { corners: [2], label: 'Bottom-right media surface corner' },
                { corners: [3], label: 'Bottom-left media surface corner' },
                { corners: [0, 1], label: 'Top media surface edge', edge: true },
                { corners: [1, 2], label: 'Right media surface edge', edge: true },
                { corners: [2, 3], label: 'Bottom media surface edge', edge: true },
                { corners: [3, 0], label: 'Left media surface edge', edge: true }
            ];
    ...
    ownsMapInput() {
        return !!this.projectController?.mediaSurfaceManager?.panel && !this.projectController?.eventManager?.eventMode;
    }

    canEdit(record) {
        return this.ownsMapInput() && !!record?.source && !record.authoring && !this.authoring && !this.destroyed
    ...
    editRecord(record) {
        ...
        if (record.source.surfaceId != null) return this.projectController?.mediaSurfaceManager?.edit(record.source.surfaceId) || false;
        const Editor = MediaSurfacePreviewManager._editorClass();
    ...
    _bindPixiOwner(owner) {
        ...
        const right = event => {
            if (this.projectController?.eventManager?.eventMode) return;
        ...
        const move = moveEvent => {
            ...
            if (handleSpec === null) {
                ...
                if (handleSpec.corners.length === 1) {
                    MediaSurfacePreviewManager._editorClass().resizeCorner(state.corners, originalCorners, handleSpec.corners[0], local, moveEvent.shiftKey);
                } else {
                    MediaSurfacePreviewManager.resizeEdge(state.corners, originalCorners,
                        handleSpec.corners, startLocal, local);
                }
                // Width/height define the anchor lift and projection. Changing
                // them here moves every untouched corner along with this one.
                this._authoringChanged(['corners']);
            }
            this._updatePixiOwner(owner);
        };
        const end = () => {
    ...
    detachPixi() {
        this._cancelActiveDrag?.();
        this._pixiHandleInput?.removeEventListener('pointerdown', this._onPixiHandleDown, true);
        this._pixiHandleInput = null;this._onPixiHandleDown = null;
    ...
    _updateDomOwner(owner) {
        ...
        const scan = MediaSurfacePreviewManager.scanlineAmount(owner.state);
        if (owner.scan) {
    ...
    _startDomDrag(event, owner, spec) {
        ...
        const move = moveEvent => {
            ...
            if (!spec) {
                ...
                if (spec.corners.length === 1) MediaSurfacePreviewManager._editorClass().resizeCorner(state.corners, originalCorners, spec.corners[0], local, moveEvent.shiftKey);
                else MediaSurfacePreviewManager.resizeEdge(state.corners, originalCorners, spec.corners, startLocal, local);
                // Width/height define the anchor lift and projection. Changing
                // them here moves every untouched corner along with this one.
                this._authoringChanged(['corners']);
            }
    ...
    _createThreeOwner(record, mapEditor3D) {
        ...
        try {
            ...
            if (!group) throw new Error('The 3D map has no media-surface group.');
            ...
            console.warn('Could not create a 3D media-surface preview:', error);
    ...
    _updateThreeOwner(owner) {
        ...
        if (owner.width !== Number(state.width) || owner.height !== Number(state.height)) {
            ...
            owner.width = Number(state.width);
            owner.height = Number(state.height);
            owner.worldShape = undefined;
        }
        const shape = JSON.stringify(state.worldCorners || null);
        if (owner.worldShape !== shape) {
            const corners = MediaSurfacePreviewManager._editorClass().worldQuad(state);
            const positions = owner.geometry.getAttribute('position');
            [0,1,3,2].forEach((corner,index) => positions.setXYZ(index,
                corners[corner].x * state.width / tileSize, -corners[corner].y * state.height / tileSize, 0));
            positions.needsUpdate = true;
            owner.geometry.computeVertexNormals();
            owner.geometry.computeBoundingBox();owner.geometry.computeBoundingSphere();
            owner.worldShape = shape;
        }
    ...
    _buildSurfaceRings(owner, group) {
        ...
        group.add(root);
        const resizeRoot=new THREE.Group();resizeRoot.name='video-surface-resize';group.add(resizeRoot);
        owner.resizeHandles={root:resizeRoot,handles:[]};
        for(const [x,y] of [[-1,-1],[1,-1],[1,1],[-1,1]]) {
            const handle=new THREE.Mesh(new THREE.BoxGeometry(1,1,1),new THREE.MeshBasicMaterial({color:0xffd529,depthTest:false,depthWrite:false}));
            handle.userData.corner={x,y};handle.userData.cornerIndex=y>0?(x<0?0:1):(x>0?2:3);handle.renderOrder=10001;resizeRoot.add(handle);owner.resizeHandles.handles.push(handle);
        }
        if(typeof RRAxisArrows3D!=='undefined'){owner.arrows=RRAxisArrows3D.create(THREE,radius*1.25,'video-surface-position');group.add(owner.arrows.root);}
        this._syncSurfaceRings(owner);
    }
    ...
    _syncSurfaceRings(owner) {
        ...
        if(owner.resizeHandles) {
            const {root,handles}=owner.resizeHandles,tile=this.tilemapManager?.TILE_SIZE||48;
            if(root.parent!==owner.mesh.parent)owner.mesh.parent?.add(root);
            root.position.copy(owner.mesh.position);root.quaternion.copy(owner.mesh.quaternion);root.visible=owner.mesh.visible;
            const size=Math.max(.035,this.mapEditor3D?.camera?.position.distanceTo(owner.mesh.position)*.004||.06);
            for(const handle of handles){const c=MediaSurfacePreviewManager._editorClass().worldQuad(owner.state)[handle.userData.cornerIndex];handle.position.set(c.x*owner.state.width/tile*owner.state.scaleX,-c.y*owner.state.height/tile*owner.state.scaleY,0);handle.scale.setScalar(size);}
        }
        if(owner.arrows){if(owner.arrows.root.parent!==owner.mesh.parent)owner.mesh.parent.add(owner.arrows.root);RRAxisArrows3D.sync(owner.arrows,owner.mesh.position,owner.mesh.visible);}
    ...
    _disposeSurfaceRings(owner) {
        if(owner.resizeHandles){owner.resizeHandles.root.removeFromParent();for(const h of owner.resizeHandles.handles){h.geometry.dispose();h.material.dispose();}owner.resizeHandles=null;}
        if(owner.arrows){RRAxisArrows3D.dispose(owner.arrows);owner.arrows=null;}
        const rings = owner.rings;
        if (!rings) return;
    ...
    _pickSurfaceResize(owner, x, y) {
        if(!owner?.resizeHandles?.root.visible)return null;
        const rect=this._threeInputRect(),camera=this.mapEditor3D?.camera;
        if(!rect||!camera)return null;
        owner.resizeHandles.root.updateMatrixWorld(true);
        let best=null;
        for(const handle of owner.resizeHandles.handles) {
            const p=handle.getWorldPosition(new THREE.Vector3()).project(camera);
            if(p.z< -1||p.z>1)continue;
            const distance=Math.hypot(x-(rect.left+(p.x+1)*rect.width/2),y-(rect.top+(1-p.y)*rect.height/2));
            if(distance>11||best&&distance>=best.distance)continue;
            const center=owner.mesh.position.clone(),normal=new THREE.Vector3(0,0,1).applyQuaternion(owner.mesh.quaternion);
            const start=this._ringPlanePoint(x,y,normal,center);
            if(!start)continue;
            best={distance,center,normal,start,turn:owner.mesh.quaternion.clone().invert(),corner:handle.position.clone(),
                index:handle.userData.cornerIndex,corners:MediaSurfacePreviewManager._editorClass().worldQuad(owner.state).map(p=>({...p})),
                width:owner.state.width,height:owner.state.height,scaleX:owner.state.scaleX,scaleY:owner.state.scaleY};
        }
        return best;
    }

    _dragSurfaceResize(owner, grab, x, y, keepProportions = false) {
        const editor=this.authoring?.editor,point=this._ringPlanePoint(x,y,grab.normal,grab.center);
        if(!editor||!point)return;
        const tile=this.tilemapManager?.TILE_SIZE||48;
        if (Math.abs(grab.width*grab.scaleX)<1e-8 || Math.abs(grab.height*grab.scaleY)<1e-8) return;
        const delta=point.sub(grab.start).applyQuaternion(grab.turn),original=grab.corners[grab.index];
        const next={x:original.x+delta.x*tile/(grab.width*grab.scaleX),
            y:original.y-delta.y*tile/(grab.height*grab.scaleY)};
        const state=this.authoring.data;
        state.worldCorners ||= [];
        MediaSurfacePreviewManager._editorClass().resizeCorner(state.worldCorners,grab.corners,grab.index,next,keepProportions);
        editor.applyLiveMapPlacement(['worldCorners']);this.updateAuthoring(editor);
    }

    _pickSurfaceArrow(owner,clientX,clientY) {
        if(!owner?.arrows||typeof RRAxisArrows3D==='undefined')return null;
        const grab=RRAxisArrows3D.pick(THREE,owner.arrows,this.mapEditor3D?.camera,this._threeInputRect(),clientX,clientY);
        if(!grab)return null;
        const field={x:'x',y:'z',z:'y'}[grab.axis];
        return {...grab,field,startValue:Number(owner.state[field])||0,
            factor:field==='z'||owner.state.target==='map'?1:(this.tilemapManager?.TILE_SIZE||48)};
    }

    _dragSurfaceArrow(owner,grab,clientX,clientY) {
        const value=grab.startValue+grab.travel(clientX,clientY)*grab.factor;if(!Number.isFinite(value))return;
        owner.state[grab.field]=Math.round(value*100)/100;
        this._updateThreeOwner(owner);this._authoringChanged([grab.field]);
    }
    ...
    _attachThreeInput(map3d) {
        ...
        this._onThreeContextMenu = event => {
            ...
            if (!this.canEdit(owner?.record)) return;
            event.preventDefault();
            event.stopImmediatePropagation();
        ...
        this._onThreePointerMove = event => {
            ...
            const arrow=this._pickSurfaceArrow(owner,event.clientX,event.clientY),over=arrow?null:this._pickSurfaceRing(owner, event.clientX, event.clientY);
            if(owner.arrows)RRAxisArrows3D.emphasize(owner.arrows,arrow?.axis||'',false);
            this._emphasizeSurfaceRing(owner, over ? over.axis : '', false);
            surface.style.cursor = this._pickSurfaceResize(owner,event.clientX,event.clientY) ? 'nwse-resize' : arrow||over ? 'grab' : '';
        };
        this._onThreePointerDown = event => {
            if (event.button !== 0 || event.altKey || event.ctrlKey
                || this.projectController?.mediaSurfaceManager?.cancelPlacement) return;
            const authoringOwner = this.authoring ? this.threeOwners.get('__authoring__') : null;
            const resizeGrab=this._pickSurfaceResize(authoringOwner,event.clientX,event.clientY);
            if(resizeGrab){
                this._cancelActiveDrag?.();event.preventDefault();event.stopImmediatePropagation();surface.style.cursor='nwse-resize';
                const move=e=>this._dragSurfaceResize(authoringOwner,resizeGrab,e.clientX,e.clientY,e.shiftKey);
                const end=()=>{window.removeEventListener('pointermove',move,true);window.removeEventListener('pointerup',end,true);window.removeEventListener('pointercancel',end,true);if(this._cancelActiveDrag===end)this._cancelActiveDrag=null;surface.style.cursor='';};
                this._cancelActiveDrag=end;window.addEventListener('pointermove',move,true);window.addEventListener('pointerup',end,true);window.addEventListener('pointercancel',end,true);return;
            }
            if (event.shiftKey) return;
            const arrowGrab=this._pickSurfaceArrow(authoringOwner,event.clientX,event.clientY);
            if(arrowGrab){
                this._cancelActiveDrag?.();event.preventDefault();event.stopImmediatePropagation();
                RRAxisArrows3D.emphasize(authoringOwner.arrows,arrowGrab.axis,true);surface.style.cursor='grabbing';
                const move=e=>this._dragSurfaceArrow(authoringOwner,arrowGrab,e.clientX,e.clientY);
                const end=()=>{window.removeEventListener('pointermove',move,true);window.removeEventListener('pointerup',end,true);window.removeEventListener('pointercancel',end,true);if(this._cancelActiveDrag===end)this._cancelActiveDrag=null;RRAxisArrows3D.emphasize(authoringOwner.arrows,'',false);surface.style.cursor='';};
                this._cancelActiveDrag=end;window.addEventListener('pointermove',move,true);window.addEventListener('pointerup',end,true);window.addEventListener('pointercancel',end,true);return;
            }
    ...
    beginAuthoring(editor, data, context) {
        ...
        if (editor.operation === 'TransformVideoSurface') {
            replaced = MediaSurfacePreviewManager.stateBefore(context, data.id);
            if (replaced) {
                const Editor = MediaSurfacePreviewManager._editorClass();
                const present = data[Editor.META]?.presentFields || new Set();
                const effective = MediaSurfacePreviewManager._copy(replaced.state);
                const sizeChanged = present.has('width') || present.has('height');
                for (const field of Editor.SHOW_FIELDS) {
                    if (field === 'movie' || field === 'wait' || !present.has(field)) continue;
                    effective[field] = MediaSurfacePreviewManager._copy(data[field]);
                }
                if (present.has('corners')) {
                    effective.corners = MediaSurfacePreviewManager._copy(data.corners);
        ...
        let replacedKey = context?.mapSurface && context?.editing
            ? `map:${this.map.id}:${context.source.surfaceId}` : replaced?.key || null;
    ...
    async navigateTo(source) {
        ...
        if (source.surfaceId != null && Number(source.mapId) === Number(this.map?.id)) {
            return this.projectController?.mediaSurfaceManager?.edit(source.surfaceId) || false;
        }
...
if (typeof globalThis !== 'undefined') globalThis.MediaSurfacePreviewManager = MediaSurfacePreviewManager;
if (typeof module !== 'undefined' && module.exports) module.exports = MediaSurfacePreviewManager;

// Legacy editor integrations share the same constructor.
if (typeof globalThis !== "undefined") globalThis.VideoSurfacePreviewManager = MediaSurfacePreviewManager;
