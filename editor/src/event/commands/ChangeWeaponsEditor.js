/**
 * ChangeWeaponsEditor - Editor for Change Weapons event command (code 127)
 */
class ChangeWeaponsEditor {
    constructor(databaseManager, projectController) {
        ...
        // Parameters: [weaponId, operation, operandType, operand, includeEquipment]
        // operation: 0=Increase, 1=Decrease
        // operandType: 0=Constant, 1=Variable
        // includeEquipment: true/false (whether to include equipped weapons when decreasing)
        this.weaponId = 1;
    ...
    show(command, callback) {
        ...
        if (command && command.code === 127) {
            const params = command.parameters;
            this.weaponId = params[0] || 1;
    ...
    createModal() {
        ...
        this.modal.className = 'change-weapons-editor-modal';
        ...
        container.className = 'change-weapons-container rr-modal';
    ...
    renderContent() {
        ...
        const container = this.modal.querySelector('.change-weapons-container');
        container.innerHTML = '';
        ...
        header.innerHTML = `
            <div class="rr-modal-title">${tt('Change Weapons')}</div>
        ...
        weaponRow.style.cssText = 'display: flex; align-items: center; gap: 8px;';

        const weaponLabel = document.createElement('span');
        weaponLabel.textContent = tt('Weapon:');
        weaponLabel.style.cssText = 'color: var(--color-text); font-size: 13px; min-width: 80px;';

        const weaponSelect = document.createElement('select');
        weaponSelect.style.cssText = `
            padding: 6px 10px;
        ...
        weapons.forEach(weapon => {
            if (weapon) {
                ...
                option.textContent = `#${weapon.id.toString().padStart(4, '0')}: ${weapon.name}`;
                weaponSelect.appendChild(option);
            }
        });

        weaponSelect.value = this.weaponId;
        weaponSelect.addEventListener('change', (e) => {
            this.weaponId = parseInt(e.target.value);
        });

        weaponRow.appendChild(weaponLabel);
        weaponRow.appendChild(weaponSelect);
        content.appendChild(weaponRow);
        ...
        if (this.operation === 1) {
            ...
            equipLabel.textContent = tt('Include equipped weapons');
    ...
    getWeapons() {
        // Get weapons from database manager
        if (this.databaseManager && this.databaseManager.data && this.databaseManager.data.weapons) {
            return this.databaseManager.data.weapons;
        }

        // Fallback: create dummy weapons list
        const weapons = [null]; // Index 0 is null
        for (let i = 1; i <= 100; i++) {
            weapons.push({
                id: i,
                name: `Weapon ${i.toString().padStart(3, '0')}`
            });
        }
        return weapons;
    }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = ChangeWeaponsEditor;
}
