/**
 * Native editor for RPG Reactor's media-surface plugin commands.
 * Commands remain ordinary MZ code-357 records so projects stay portable.
 */
class MediaSurfaceEditor {
    ...
    static get ALIASES() {
        return {
            ...
            corners: ['corners', 'fourCorners'], worldCorners: ['worldCorners']
        };
    }

    static get SHOW_FIELDS() {
        return ['target', 'movie', 'eventId', 'x', 'y', 'z', 'width', 'height', 'opacity',
            'loop', 'muted', 'volume', 'playbackRate', 'corners', 'worldCorners', 'rotationX', 'rotationY',
    ...
    static parse(command, operation) {
        ...
        if (name !== 'StopVideoSurface') {
            ...
            const worldCorners = this._field(rawArgs, 'worldCorners');
            if (worldCorners.present) data.worldCorners = this.jsonValue(worldCorners.value);
    ...
    static build(operation, data, indent = 0, options = {}) {
        if (!this.supports(operation)) throw new TypeError(`Unknown media-surface operation: ${operation}`);
        ...
        if (operation === 'StopVideoSurface') {
            ...
            for (const key of this.SHOW_FIELDS) {
                ...
                if (key === 'worldCorners' && data.worldCorners == null) continue;
    ...
    static validate(operation, data, context = {}, changedFields = null) {
        ...
        if ((!changed || changed.has('worldCorners')) && data.worldCorners != null &&
            (!Array.isArray(data.worldCorners) || data.worldCorners.length !== 4 ||
                data.worldCorners.some(p => !Number.isFinite(p?.x) || !Number.isFinite(p?.y)))) {
    ...
    movieFiles(project = this._project()) {
        ...
        if (assets?.list) {
            return assets.list(root, ['.webm', '.mp4'], { anyCase: true })
                .filter(file => MediaSurfaceEditor.safeMoviePath(file.relativePath))
        ...
        const visit = (directory, parts) => {
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) visit(absolutePath, parts.concat(entry.name));
                    ...
                    if (MediaSurfaceEditor.safeMoviePath(relativePath)) result.push({ relativePath, absolutePath });
                }
            }
        };
        visit(root, []);
        ...
        if (!options.manual) {
            ...
            const update = final => {
                ...
                this.data[key] = options.numeric || input.type === 'number'
                    ? MediaSurfaceEditor.number(input.value, previous, !!options.integer)
                    : input.value;
    ...
    // Metadata has to load independently of the workspace preview: live map
    // authoring has no local video/image elements. Never resize saved geometry
    // merely because its inspector was opened.
    _loadMediaDimensions(fit = false) {
        this._cancelMediaDimensions?.();
        this.mediaDimensions = null;
        const asset = this._movieAsset(), data = this.data;
        if (!asset) return Promise.resolve(false);
        const image = MediaSurfaceEditor.isImageFile(data.movie);
        const probe = document.createElement(image ? 'img' : 'video');
        const event = image ? 'load' : 'loadedmetadata';
        const geometry = () => JSON.stringify([data.width, data.height, data.scaleX, data.scaleY, data.corners, data.worldCorners]);
        ...
        return new Promise(resolve => {
            ...
            const finish = result => {
                if (!active) return;
                active = false;
                probe.removeEventListener(event, loaded);
                probe.removeEventListener('error', failed);
                probe.removeAttribute('src');
                if (!image) probe.load?.();
                if (this._cancelMediaDimensions === cancel) this._cancelMediaDimensions = null;
                resolve(result);
            };
            const cancel = () => finish(false), failed = () => finish(false);
            const loaded = () => {
                if (!active || this.data !== data) return finish(false);
                const width = image ? probe.naturalWidth : probe.videoWidth;
                const height = image ? probe.naturalHeight : probe.videoHeight;
                if (!(width > 0 && height > 0)) return finish(false);
                this.mediaDimensions = { width, height };
                // A slow file must not overwrite resizing done while it loads.
                if (fit && this.keepProportions !== false && geometry() === before) this._fitMediaProportions();
                finish(true);
            };
            this._cancelMediaDimensions = cancel;
            probe.addEventListener(event, loaded);
            probe.addEventListener('error', failed);
            if (!image) { probe.preload = 'metadata'; probe.muted = true; }
            probe.src = typeof RRAssetFiles !== 'undefined' && RRAssetFiles.toUrl
                ? RRAssetFiles.toUrl(asset.absolutePath) : asset.absolutePath;
        });
    }

    _fitMediaProportions() {
        if (!this.mediaDimensions || !this.data) return;
        const oldWidth = this.data.width, oldHeight = this.data.height;
        const ratio = this.mediaDimensions.width / this.mediaDimensions.height;
        const longest = Math.min(8192, Math.max(1, oldWidth, oldHeight));
        this.data.width = Math.max(1, ratio >= 1 ? longest : longest * ratio);
        this.data.height = Math.max(1, ratio >= 1 ? longest / ratio : longest);
        this._resizeCorners(oldWidth, oldHeight);
        for (const key of ['width', 'height']) {
            if (this.fields[key]) this.fields[key].value = this.data[key];
            this._markChanged(key);
        }
        this._updatePreviews();
    }

    _resizeDimension(key, previous) {
        const other = key === 'width' ? 'height' : 'width';
        const oldWidth = key === 'width' ? previous : this.data.width;
        const oldHeight = key === 'height' ? previous : this.data.height;
        const ownsOther = this.operation !== 'TransformVideoSurface'
            || this.data[MediaSurfaceEditor.META]?.presentFields?.has(other) || this.changedFields.has(other);
        if (this.keepProportions !== false && ownsOther && previous > 0 && this.data[other] > 0) {
            const factor = Math.max(1 / previous, 1 / this.data[other],
                Math.min(this.data[key] / previous, 8192 / previous, 8192 / this.data[other]));
            this.data[key] = previous * factor;
            this.data[other] *= factor;
            for (const field of [key, other]) {
                if (this.fields[field]) this.fields[field].value = this.data[field];
    ...
    _setMovie(fit = false) {
        this.mediaDimensionsReady = this._loadMediaDimensions(fit);
        ...
        if (MediaSurfaceEditor.isImageFile(this.data.movie)) {
    ...
    _resizeCorners(oldWidth, oldHeight) {
        const meta = this.data?.[MediaSurfaceEditor.META];
    ...
    _buildControls(parent) {
        ...
        this._field(identityGrid, 'Target', 'target', {
            select: this.context.mapSurface ? [['map', 'Fixed Map Position']] : targets,
            onChange: (value, previous) => {
                this._convertPositionForTarget(previous);
                this._syncWorkspaceMode();
            }
        });
        if (!this.context.mapSurface) this._buildEventField(identityGrid);
        ...
        if (this.operation === 'ShowVideoSurface') {
            ...
            kind.value = MediaSurfaceEditor.isImageFile(this.data.movie) ? 'image' : 'video';
        ...
        for(const [label,key,color] of [['X','x','#ff5c5c'],['Y','y','#5ca8ff'],['Z / Elevation','z','#3ddc84']]){
            const input=this._field(bindingGrid,label,key,{step:0.1});
            const slider=this._el('input','vs-position-slider');slider.type='range';slider.step=.01;slider.dataset.key=key;slider.setAttribute('aria-label',this._t(label));slider.style.cssText='width:100%;min-width:0;margin:5px 0 0;accent-color:'+color;
            this.positionSliders[key]=slider;input.closest('.vs-field').appendChild(slider);
            this._listen(slider,'input',()=>{this.data[key]=Number(slider.value);input.value=slider.value;this._markChanged(key);this._updatePreviews();});
            this._listen(slider,'change',()=>this._syncPositionSliders(true));
        }
        this._syncPositionSliders(true);
        this._syncTargetFields();
        this._field(bindingGrid, 'Width', 'width', {
            min: 1, max: 8192, step: 0.1,
            onChange: (value, previous) => this._resizeDimension('width', previous)
        });
        this._field(bindingGrid, 'Height', 'height', {
            min: 1, max: 8192, step: 0.1,
            onChange: (value, previous) => this._resizeDimension('height', previous)
        });

        const placement3d = this._section(parent, '3D World Transform');
        const linked = this._el('label', 'vs-check');
        linked.style.cssText = 'display:flex;align-items:center;gap:7px;margin-bottom:10px;color:var(--color-text);font-size:12px;';
        const keep = this._el('input'); keep.type = 'checkbox'; keep.checked = this.keepProportions !== false;
        this.keepProportions = keep.checked;
        this._listen(keep, 'change', () => {
            this.keepProportions = keep.checked;
            if (keep.checked) {
                if (this.mediaDimensions) this._fitMediaProportions();
                else this.mediaDimensionsReady = this._loadMediaDimensions(true);
            }
        });
        linked.title = this._t("Uses the media's original proportions and links size and scale controls. Hold Shift while dragging a corner to keep proportions.");
        linked.append(keep, this._el('span', '', this._t('Keep Proportions'))); placement3d.append(linked);
        const scaleGrid = this._grid(placement3d);
        scaleGrid.style.gridTemplateColumns = 'repeat(2,minmax(0,1fr))';
        const transformGrid = this._grid(placement3d);
        this.transformSliders = {};
        for (const [label, key, color] of [
            ['Scale X','scaleX','#ffd529'],['Scale Y','scaleY','#ffd529'],
            ['Rotation X','rotationX','#ff5c5c'],['Rotation Y','rotationY','#3ddc84'],['Rotation Z','rotationZ','#5ca8ff']
        ]) {
            const scale = key.startsWith('scale');
            const input = this._field(scale?scaleGrid:transformGrid,label,key,{min:scale?-1000:-360000,max:scale?1000:360000,step:scale?.01:.1,
                onChange:(value,previous)=>{ if(scale)this._linkSurfaceScale(key,value,previous); }});
            const slider = this._el('input','vs-transform-slider'); slider.type='range'; slider.step=scale?.01:.1;
            slider.dataset.key=key; slider.setAttribute('aria-label',this._t(label));
            slider.style.cssText='width:100%;min-width:0;margin:5px 0 0;accent-color:'+color;
            this.transformSliders[key]=slider;input.closest('.vs-field').append(slider);
            this._listen(slider,'input',()=>{input.value=slider.value;input.dispatchEvent(new Event('input',{bubbles:true}));});
            this._listen(slider,'change',()=>this._syncTransformSliders(true));
        }
        this._syncTransformSliders(true);
        ...
        if (this.operation === 'ShowVideoSurface' && this.context.type !== 'troop' && !this.context.mapSurface) {
        ...
        if (this._mapIs3D()) corners.style.display = 'none';
        ...
        this.data.corners.forEach((corner, index) => {
            for (const axis of ['x', 'y']) {
                ...
                const update = () => {
                    const point = this.data.corners[index];
                    point[axis] = MediaSurfaceEditor.number(input.value, point[axis]);
                    this._markChanged('corners');
                    this._updatePreviews();
                };
    ...
    _syncMediaFields() {
        const isImage = MediaSurfaceEditor.isImageFile(this.data.movie);
    ...
    _buildEventField(parent) {
        ...
        const options = mapEvents.map(event => [String(event.id), MediaSurfaceEditor._eventLabel(event)]);
    ...
    _syncTargetFields() {
        ...
        if (eventInput) {
            ...
            if (editable) {
                ...
                this.eventIndicator.this.textContent = thisEvent ? MediaSurfaceEditor._eventLabel(thisEvent) : this._t('None');
    ...
    _syncPositionSliders(recenter=false) {
        const three=this._usesThree();
        for(const [key,slider] of Object.entries(this.positionSliders||{})){
            slider.style.display=three?'':'none';
            const value=Number(this.data[key])||0,span=key==='z'||this.data.target==='map'?8:8*(this.projectController?.tilemapManager?.TILE_SIZE||48);
            if(recenter||slider._target!==this.data.target||value<Number(slider.min)||value>Number(slider.max)||!slider._ready){
                slider.min=Math.floor((value-span)*10)/10;slider.max=Math.ceil((value+span)*10)/10;slider._ready=true;slider._target=this.data.target;
            }
            slider.value=value;
        }
    }
    ...
    // Optional normalized local coordinates. Older 3D surfaces keep their
    // rectangular geometry; their independent 2D warp is not reinterpreted.
    static worldQuad(data) {
        return data.worldCorners || [{x:-.5,y:-.5},{x:.5,y:-.5},{x:.5,y:.5},{x:-.5,y:.5}];
    }

    // Scale the original quad about its opposite corner while Shift is held.
    // Rebuild from the drag snapshot so releasing Shift restores free warping
    // without leaving the other corners at their last constrained positions.
    static resizeCorner(corners, original, index, point, keepProportions = false) {
        for (let i = 0; i < original.length; i++) corners[i] = { ...original[i] };
        if (!keepProportions) {
            corners[index] = { ...point };
            return;
        }
        const anchor = original[(index + 2) % 4];
        const dx = original[index].x - anchor.x, dy = original[index].y - anchor.y;
        const lengthSq = dx * dx + dy * dy;
        if (lengthSq < 1e-8) return;
        const factor = Math.max(.001, ((point.x - anchor.x) * dx + (point.y - anchor.y) * dy) / lengthSq);
        for (let i = 0; i < original.length; i++) {
            corners[i] = {
                x: anchor.x + (original[i].x - anchor.x) * factor,
                y: anchor.y + (original[i].y - anchor.y) * factor
            };
        }
    }
    ...
    _bindPlacementPointers(surface, plane) {
        const startDrag = (event, corner) => {
            ...
            const originalCorners = this.data.corners.map(point => ({ ...point }));
            ...
            const move = moveEvent => {
                ...
                if (corner === null) {
                    ...
                    MediaSurfaceEditor.resizeCorner(this.data.corners, originalCorners, corner,
                        this._localFromWorkspace(point), moveEvent.shiftKey);
                    this._markChanged('corners');
                    this._syncCornerFields();
                }
                this._updatePreviews();
    ...
    _linkSurfaceScale(key, value, previous) {
        if (!this.keepProportions || !Number.isFinite(previous) || Math.abs(previous)<1e-8) return;
        const other = key==='scaleX'?'scaleY':'scaleX';
        this.data[other] = Math.max(-1000,Math.min(1000,this.data[other]*value/previous));
        if(this.fields[other])this.fields[other].value=String(this.data[other]);
        this._markChanged(other);
    }

    _syncTransformSliders(recenter=false) {
        for(const [key,slider] of Object.entries(this.transformSliders||{})) {
            const value=Number(this.data?.[key])||0,scale=key.startsWith('scale');
            if(recenter||!slider._ready||value<Number(slider.min)||value>Number(slider.max)) {
                if(scale){slider.min=Math.min(value,0);slider.max=Math.max(4,Math.ceil(value));}
                else {slider.min=Math.floor((value-180)/10)*10;slider.max=Number(slider.min)+360;}
                slider._ready=true;
            }
            slider.value=value;
        }
    ...
    _updatePreviews() {
        if (!this.data) return;
        this._syncPositionSliders();
        this._syncTransformSliders();
    ...
    _previewManager() {
        return this.projectController?.mediaSurfacePreviewManager
            || (typeof window !== 'undefined' && window.reactor?.mediaSurfacePreviewManager) || null;
    }
    ...
    _popOut() {
        ...
        nw.Window.open('media-surface-panel.html', {
            ...
            const mount = () => {
                ...
                doc.title = this._t(this.context.mapSurface ? 'Map Media Surface' : MediaSurfaceEditor.OPERATIONS[this.operation]);
    ...
    _makePanelDraggable(header, dialog, closeButton) {
        ...
        let offset = { x: 0, y: 0 };
        try {
            const saved = JSON.parse(localStorage.getItem(MediaSurfaceEditor.PANEL_OFFSET_KEY) || 'null');
        ...
        this._listen(header, 'pointerdown', event => {
            ...
            let end;
                ...
                window.removeEventListener('pointerup', end);
                window.removeEventListener('pointercancel', end);
                try { localStorage.setItem(MediaSurfaceEditor.PANEL_OFFSET_KEY, JSON.stringify(offset)); } catch (_) {}
            });
            window.addEventListener('pointermove', move);
            window.addEventListener('pointerup', end);
            window.addEventListener('pointercancel', end);
    ...
    _resolveBinding() {
        this.data.id = MediaSurfaceEditor.number(this.fields.id?.value, this.data.id, true);
    ...
    show(command, callback, operation, context = null) {
        ...
        if (!MediaSurfaceEditor.supports(this.operation)) return false;
        if (command && !this.acceptsCommand(command)) return false;
        this.context = Object.assign({ type: 'map' }, context || {});
        this.callback = callback;
        this.data = MediaSurfaceEditor.parse(command, this.operation);
        this.rawArgs = this.data?.[MediaSurfaceEditor.META]?.rawArgs || {};
        this.changedFields = new Set();
        ...
        header.appendChild(this._el('div', 'rr-modal-title', this._t(this.context.mapSurface ? 'Map Media Surface' : MediaSurfaceEditor.OPERATIONS[this.operation])));
        ...
        if (this.liveMapAuthoring) {
            const help = this._el('div', 'vs-live-map-help', this._t(this._mapIs3D()
                ? 'Drag arrows to move, rings to rotate, and yellow corners to resize. Hold Shift while resizing to keep proportions.'
                : 'Drag the media surface on the map to move it. Drag a corner handle to warp it or an edge handle to resize it. Hold Shift while dragging a corner to keep proportions. Drag this panel by its title to uncover the map. Right-click saved surfaces to open their source command.')
                + (this._mapIs3D() ? ' ' + this._t('Ctrl + right-drag: look around. WASD: move. Q/E: down/up.') : ''));
        ...
        if (this.context.fromMap && this.context.source && !this.context.mapSurface) {
        ...
        const accept = after => {
            ...
            const errors = MediaSurfaceEditor.validate(this.operation, this.data, this.context,
                this.operation === 'TransformVideoSurface' ? this.changedFields : null);
            if (this.context.validate) errors.push(...this.context.validate(this.data));
            if (errors.length) {
                this._showValidation(errors);
                return;
            }
            const built = MediaSurfaceEditor.build(this.operation, this.data, command?.indent || 0, {
        ...
        this._listen(ok, 'click', () => accept());
        if (this.context.mapSurface && this.context.duplicate) {
            const duplicate = this._button(typeof window !== 'undefined' && window.I18n ? window.I18n.t('common.duplicate') : 'Duplicate');
            duplicate.dataset.action = 'duplicate-surface';
            duplicate.style.marginRight = 'auto';
            const onDuplicate = this.context.duplicate;
            this._listen(duplicate, 'click', () => accept(onDuplicate));
            footer.prepend(duplicate);
        }
        this._setMovie();
        this._updatePreviews();
        return true;
    ...
    close(notify = false) {
        ...
        for (const key of ['modal', 'video', 'video3d', 'workspace', 'quadSurface', 'handles',
            'perspective', 'plane3d', 'sliders', 'positionSliders', 'transformSliders', 'eventAnchor', 'coordinateLabel', 'previewMode',
...
if (typeof globalThis !== 'undefined') globalThis.MediaSurfaceEditor = MediaSurfaceEditor;
if (typeof module !== 'undefined' && module.exports) module.exports = MediaSurfaceEditor;

// Legacy editor integrations share the same constructor.
if (typeof globalThis !== "undefined") globalThis.VideoSurfaceEditor = MediaSurfaceEditor;
