...
test('the frame loop and thumbnail pass are wired to the gates', () => {
    ...
    assert.match(source, /async _drawPreview\(entry\) \{\s*const gen = \+\+this\._gen;\s*this\._loadingPreview = true;\s*this\._refreshHint\(\);\s*try \{\s*return await this\._drawPreviewNow\(entry, gen\);\s*\} finally \{\s*if \(gen === this\._gen\) \{\s*this\._loadingPreview = false;\s*this\._refreshHint\(\);/);
