...
(async()=>{try {
    ...
    const pixels=await driver.executeAsync(`
        ...
        (async()=>{
            ...
            for(const [name,values] of [['dark',dark],['lit',lit]]){
                ...
                require('fs').writeFileSync('/tmp/rr-flat-reactor-'+name+'.png',Buffer.from(canvas.toDataURL().split(',')[1],'base64'));
            }
    ...
    console.log('Flat model lighting passed: actual reactor surface response, no floor footprint required, isolated uniforms, reset and single ambient application.');
