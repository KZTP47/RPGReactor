...
window.__auditTheme = function () {
 ...
 const low=[],missing=[],offPaletteButtons=[];let sampled=0;
 for(const e of document.querySelectorAll('[class*="modal"] input,[class*="modal"] textarea,[class*="modal"] select,[class*="modal"] button,[class*="modal"] label,[class*="modal"] p,[class*="modal"] span,.rr-database-action-menu button,.rr-database-action-menu span')){
  ...
  if(e.tagName==='BUTTON'&&/var\(--color-(?:link|info)(?:-bright)?\)/.test(e.style.background+e.style.backgroundColor))offPaletteButtons.push({text:e.textContent.trim(),background:e.style.backgroundColor});
 ...
 return {sampled,low,tokens,missing,offPaletteButtons};
};
