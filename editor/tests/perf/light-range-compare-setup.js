...
// Compares the shipped shader with its light range rejection disabled, holding
...
(async () => {
    ...
    function source(shadows, taps) {
        ...
        const guard = '\t\tif (max(max(abs(d.x), abs(d.y)), abs(d.z)) >= max(lp.w, 0.001)) continue;\n';
        if (!s.includes(guard)) throw new Error('Expected the production light range rejection');
        if (variant === 'baseline') s = s.replace(guard, '');
        return s;
    }
    function draw() {
