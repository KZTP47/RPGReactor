...
// Compares the shipped shader with its spotlight early exit disabled, holding
// scene state, clocks, media and quality constant. See docs/PERFORMANCE.md.
...
(async () => {
    ...
    const videos = Array.from(globalThis.RPGReactorVideoSurfaces?.manager?._owners?.values() || [])
    ...
    function source(shadows, taps) {
        let s = original.call(R, shadows, taps);
        const spot = 'fall *= smoothstep(aim.w, mix(aim.w, 1.0, 0.35), c);';
        const guarded = spot + '\n\t\t\tif (fall <= 0.0) continue;';
        if (!s.includes(guarded)) throw new Error('Expected the production spotlight early exit');
        if (variant === 'baseline') s = s.replace(guarded, spot);
        return s;
    }
    function draw() {
    ...
    try {
        ...
        for (const name of ['optimized', 'baseline', 'optimized']) {
        ...
        const report = {
            ...
            passed: materials.size > 0 && base.image.some((x, i) => i % 4 !== 3 && x !== base.image[i % 4]) &&
                results.every(r => r.glError === 0 && r.gpu.n === 40 && (!r.pixels || r.pixels.changedChannels === 0))
        };
